...
const mockGroups: GroupItem[] = [
  { id: "1", name: "默认分组", accountCount: 401, isDefault: true },
  { id: "2", name: "B", accountCount: 0 },
...
function NewGroupDialog({ open, onOpenChange, onSave }: { open: boolean; onOpenChange: (v: boolean) => void; onSave: (name: string) => void }) {
  ...
  const handleConfirm = () => {
    ...
    if (name.trim().length > 30) { toast.error("分组名称不能超过30个字符"); return; }
    onSave(name.trim());
    setName("");
    onOpenChange(false);
  };
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 py-2">
          ...
          <div className="relative">
            <Input placeholder="请输入分组名称" value={name} onChange={(e) => setName(e.target.value.slice(0, 30))} maxLength={30} />
...
function EditGroupDialog({ open, onOpenChange, group, onSave }: { open: boolean; onOpenChange: (v: boolean) => void; group: GroupItem | null; onSave: (id: string, name: string) => void }) {
  const [name, setName] = useState(group?.name || "");

  const handleConfirm = () => {
    if (!name.trim()) { toast.error("请输入分组名称"); return; }
    if (name.trim().length > 30) { toast.error("分组名称不能超过30个字符"); return; }
    if (group) onSave(group.id, name.trim());
    setName("");
    onOpenChange(false);
  };
...
export function GroupManageDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const handleEditGroup = (id: string, name: string) => {
    ...
    toast.success(`分组已更新为「${name}」`);
  };

  const handleDelete = (group: GroupItem) => {
    if (group.accountCount > 0) {
      toast.error("该分组下存在账号，无法删除");
      return;
    }
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            <Label className="whitespace-nowrap shrink-0 text-sm text-muted-foreground">分组</Label>
                    ...
                    <TableCell className="text-center">
                      {g.isDefault ? null : (
                        <div className="flex items-center justify-center gap-3">
                          <Button variant="link" className="text-primary p-0 h-auto" onClick={() => setEditGroup(g)}>编辑</Button>
                          <Button variant="link" className="text-destructive p-0 h-auto" onClick={() => handleDelete(g)}>删除</Button>
                        </div>
                      )}
                    </TableCell>
                  </TableRow>
      ...
      <EditGroupDialog open={!!editGroup} onOpenChange={(v) => !v && setEditGroup(null)} group={editGroup} onSave={handleEditGroup} />
    </>
  );
}
